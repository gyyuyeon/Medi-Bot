...
def pham_loc():
    ...
    user_input = req['action']['params']['pham_loc'] # 사용자 입력 위치   
...
# 약국조회 파트     geo_code(사용자 입력주소 좌표변환), forlium(지도 시각화) 
...
# def response_pham_loc(user_input):        
#     responseBody = {"version": "2.0", "template": {"outputs": []}}  # 응답 초기화
#     responseBody = {
#         "version": "2.0",
#         "template": {
#             "outputs": [
...
# 약 이미지로 검색 파트 - server2로 request 요청및 값을 json형식으로 send로 가져와서 리스트형식으로 변환후 image_to_text.txt 에 값 저장
...
# 약 이름으로 데이터 셋에서 검색하여 데이터 반환 함수 모음
# API에서 데이터 가져오기
...
def data_process(user_input):
    ...
    mdcin_params = {
        'serviceKey': 'FsAw2Tgk2O1XaJ11I2HTvPmbhF+hAaFmg61CyViMSq6lC4V6PWH9ZUAt1Iclw17n4+cAYTwrSZ160cE4IOIcw==',
...
def detail_info(name_input, text_input, index):
    ...
    #print('상세 user_input:', name_input, '/', text_input)
...
def all_list(name_input, text_input):
    ...
    print('전체 user_input:')#, name_input, '/', text_input)
    ...
    if text_input=='0':
        ...
        valid_drugs = search_medicine(text_input)
        user_input=text_input
    print('각인 전체 리스트:')#, valid_drugs)
    
    data, _, _ = data_dict(valid_drugs)
    name_list = data["item_namelist"]
    img_list = data["item_image_urllist"]

    print('전제 약품이름 리스트: ')#,name_list)
